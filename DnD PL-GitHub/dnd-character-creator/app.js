...
class CharacterApp {
    ...
    getDefaultCharacter() {
        return {
            ...
            background: '',
            abilities: {
                strength: { base: 10, racial: 0 },
                dexterity: { base: 10, racial: 0 },
                constitution: { base: 10, racial: 0 },
                intelligence: { base: 10, racial: 0 },
                wisdom: { base: 10, racial: 0 },
                charisma: { base: 10, racial: 0 }
            },
            scoreSets: [],
    ...
    updateCharacterSheet() {
        ...
        const maxHP = hitDice + conMod + (this.character.level - 1) * (Math.floor(hitDice / 2) + 1 + conMod);
        document.getElementById('sheetHP').textContent = `${maxHP}/${maxHP}`;
    ...
    renderWeapons() {
        ...
        // 渲染近战武器
        if (meleeContainer) {
            meleeContainer.innerHTML = this.character.weapons.melee.map(weapon => `
                ...
                <div class="section">
                    ...
                    <div class="combat-info">
                        ...
                        <div class="combat-box">
                            ...
                            ${document.getElementById('sheetHP').textContent}
                        </div>
                        <div class="combat-box">
                            <strong>先攻</strong><br>
                            ${document.getElementById('sheetInitiative').textContent}
    ...
    extractSpellsFromGrant(grantsData, sourceName, characterLevel, spellsArray) {
        // 处理戏法
        if (grantsData.cantrips) {
            if (Array.isArray(grantsData.cantrips)) {
                // 固定法术列表
                grantsData.cantrips.forEach(spell => {
            ...
            } else if (grantsData.cantrips.count && grantsData.cantrips.choose) {
                // 需要选择的戏法 - 检查是否已选择
                const choiceKey = `spellChoice_${sourceName}_cantrips`;
                if (this.character.spellChoices && this.character.spellChoices[choiceKey]) {
                    // 已选择，添加选择的法术
                    const selectedSpells = this.character.spellChoices[choiceKey];
                    selectedSpells.forEach(spellName => {
                        spellsArray.push({
                            ...
                            source: sourceName,
                            fromFeature: true,
                            ability: grantsData.cantrips.ability,
                            alwaysKnown: true
                        });
                    });
                } else {
                    // 未选择，添加占位符提示
                    for (let i = 0; i < grantsData.cantrips.count; i++) {
                        spellsArray.push({
                            name: `[请选择${grantsData.cantrips.spellList === 'warlock' ? '邪术师' : ''}戏法]`,
                            level: 0,
                            school: '未知',
                            source: sourceName,
                            fromFeature: true,
                    ...
                    // 显示选择提示
                    this.showSpellChoiceDialog('cantrips', grantsData.cantrips, sourceName);
                }
            }
        }
        
        // 处理1环法术
        if (grantsData.level1) {
            if (Array.isArray(grantsData.level1)) {
                grantsData.level1.forEach(spell => {
                    if (!spell.levelRequired || characterLevel >= spell.levelRequired) {
                        spellsArray.push({
                            name: spell.name,
                            level: 1,
            ...
            } else if (grantsData.level1.count && grantsData.level1.choose) {
                // 需要选择的1环法术 - 检查是否已选择
                const choiceKey = `spellChoice_${sourceName}_level1`;
                if (this.character.spellChoices && this.character.spellChoices[choiceKey]) {
                    // 已选择，添加选择的法术
                    const selectedSpells = this.character.spellChoices[choiceKey];
                    selectedSpells.forEach(spellName => {
                        spellsArray.push({
                            ...
                            source: sourceName,
                            fromFeature: true,
                            ability: grantsData.level1.ability,
                            alwaysPrepared: grantsData.level1.alwaysPrepared,
                            freeCast: grantsData.level1.freeCast
                        });
                    });
                } else {
                    // 未选择，添加占位符提示
                    for (let i = 0; i < grantsData.level1.count; i++) {
                        spellsArray.push({
                            name: `[请选择${grantsData.level1.spellList === 'warlock' ? '邪术师' : ''}1环法术]`,
                            level: 1,
                            school: '未知',
                            source: sourceName,
                            fromFeature: true,
                    ...
                    // 显示选择提示
                    this.showSpellChoiceDialog('level1', grantsData.level1, sourceName);
                }
            }
        }
        
        // 处理2环法术
        if (grantsData.level2) {
            if (Array.isArray(grantsData.level2)) {
                grantsData.level2.forEach(spell => {
                    if (!spell.levelRequired || characterLevel >= spell.levelRequired) {
                        spellsArray.push({
                            name: spell.name,
                            level: 2,
